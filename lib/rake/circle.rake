...
def start_sauce_connect
  ...
  Dir.chdir(Dir.glob('sc-*-linux')[0]) do
    ...
    RakeUtils.exec_in_background "for i in 1 2; do ./bin/sc -vv -l $CIRCLE_ARTIFACTS/sc.log -u $SAUCE_USERNAME -k $SAUCE_ACCESS_KEY -i #{CDO.circle_run_identifier} --tunnel-domains localhost-studio.code.org,localhost.code.org --no-proxy-caching && break; done"
  end
end
